...
<div class="space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <div class="flex items-center gap-2 mb-0.5">
                    <h1 class="text-xl md:text-2xl font-black text-white tracking-tight">Edit Coupon</h1>
                    <span class="px-2.5 py-0.5 rounded-md text-xs font-mono font-black tracking-wider bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                        {{ $coupon->code }}
                    </span>
                </div>
                <p class="text-slate-400 text-xs">Update discount parameters, usage limits, and shopping channel restrictions.</p>
            </div>
        </div>

        <div class="flex items-center gap-2">
            <a href="{{ route('admin.coupons.show', $coupon) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-xs transition">
    ...
    <!-- Coupon Form -->
    <form method="POST" action="{{ route('admin.coupons.update', $coupon) }}" class="space-y-6" id="couponForm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
                    ...
                    <!-- Code -->
                    <div>
                        <label for="code" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">
                        ...
                        <p class="text-[11px] text-slate-500 mt-1">Must be unique across all active promotional vouchers.</p>
                        @error('code')
                    ...
                    <div>
                        ...
                        <input id="name" type="text" name="name" value="{{ old('name', $coupon->name) }}" required
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                            placeholder="e.g. Valid on all groceries with minimum purchase of ₹299."
                            class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition @error('description') border-rose-500 @enderror">{{ old('description', $coupon->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div id="valueWrapper">
                            ...
                            <input id="value" type="number" step="0.01" min="0" name="value" value="{{ old('value', (float) $coupon->value) }}" required
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div id="maxCapWrapper">
                            ...
                            <input id="max_discount_amount" type="number" step="0.01" min="0" name="max_discount_amount" value="{{ old('max_discount_amount', $coupon->max_discount_amount ? (float) $coupon->max_discount_amount : '') }}"
                        ...
                        <div>
                            ...
                            <input id="min_order_amount" type="number" step="0.01" min="0" name="min_order_amount" value="{{ old('min_order_amount', (float) $coupon->min_order_amount) }}"
                                placeholder="e.g. 299"
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
                    ...
                    <div>
                        ...
                        <select id="mode_id" name="mode_id" class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition @error('mode_id') border-rose-500 @enderror">
                            ...
                            @foreach ($modes as $mode)
                                <option value="{{ $mode->id }}" {{ old('mode_id', $coupon->mode_id) == $mode->id ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input id="usage_limit_per_user" type="number" min="1" name="usage_limit_per_user" value="{{ old('usage_limit_per_user', $coupon->usage_limit_per_user) }}"
                                placeholder="e.g. 1"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-slate-500 mt-1">Leave blank to keep active.</p>
                            @error('starts_at')
                                <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
                    ...
                    <div class="flex items-center justify-between p-3.5 rounded-2xl bg-slate-950 border border-slate-800">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" id="status" name="status" value="1" {{ old('status', $coupon->status) ? 'checked' : '' }} class="sr-only peer">
                            <div class="w-11 h-6 bg-slate-800 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-600"></div>
                        </label>
                    </div>

                    <!-- Usage Summary Pill -->
                    <div class="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-xs text-slate-400 space-y-1.5">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-slate-300">{{ $coupon->created_at->format('d M Y, h:i A') }}</span>
                        </div>
                    </div>
                </div>
