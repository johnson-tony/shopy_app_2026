...
@section('title', 'Add Delivery Partner — Fleet Management')
...
<div class="space-y-6 max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('admin.delivery_partners.index') }}" class="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white transition mb-2">
                <i class="fa-solid fa-arrow-left"></i> Back to Delivery Fleet
            </a>
            <h1 class="text-2xl sm:text-3xl font-black text-white tracking-tight">Add Delivery Partner</h1>
            <p class="text-slate-400 text-sm mt-1">Directly onboard a delivery driver to your logistics fleet with verified KYC &amp; channel authorizations.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.delivery_partners.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
            <div class="pb-4 border-b border-slate-800 flex items-center justify-between">
                <h2 class="text-base font-black text-white flex items-center gap-2">
                    <i class="fa-solid fa-user-circle text-indigo-400"></i> Partner Identity &amp; Login
                </h2>
                <span class="text-xs text-slate-500">* Required fields</span>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g. Ramesh Kumar" required
                        class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none transition">
                </div>
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="e.g. ramesh@delivery.com" required
                        class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none transition">
                </div>
                ...
                <div>
                    ...
                    <input type="tel" name="phone" id="phone" value="{{ old('phone') }}" placeholder="e.g. +91 98765 43210" required
                        class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none transition">
                </div>
                ...
                <div class="md:col-span-2">
                    <label for="status" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
                        Initial Account Status <span class="text-rose-400">*</span>
                    </label>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <label class="flex items-center gap-3 p-3 rounded-xl bg-slate-950 border border-slate-800 hover:border-indigo-500/50 cursor-pointer transition">
                            ...
                            <div>
                                ...
                                <span class="text-[11px] text-slate-400">Ready to accept deliveries immediately</span>
                            </div>
                        </label>
                        <label class="flex items-center gap-3 p-3 rounded-xl bg-slate-950 border border-slate-800 hover:border-amber-500/50 cursor-pointer transition">
                            <input type="radio" name="status" value="pending_approval" {{ old('status') === 'pending_approval' ? 'checked' : '' }}
                            ...
                            <div>
                                ...
                                <span class="text-[11px] text-slate-400">Requires review before dispatching</span>
                            </div>
                        </label>
                        <label class="flex items-center gap-3 p-3 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer transition">
                            <input type="radio" name="status" value="suspended" {{ old('status') === 'suspended' ? 'checked' : '' }}
                            ...
                            <div>
                                ...
                                <span class="text-[11px] text-slate-500">Temporarily disabled account</span>
                            </div>
                        </label>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="vehicle_type" id="vehicle_type" required
                        ...
                        <option value="bike" {{ old('vehicle_type', 'bike') === 'bike' ? 'selected' : '' }}>Motorcycle / Motorbike</option>
                        <option value="scooter" {{ old('vehicle_type') === 'scooter' ? 'selected' : '' }}>Scooter / Activa</option>
                        <option value="ev" {{ old('vehicle_type') === 'ev' ? 'selected' : '' }}>Electric Vehicle (EV)</option>
                ...
                <div>
                    ...
                    <input type="text" name="vehicle_number" id="vehicle_number" value="{{ old('vehicle_number') }}" placeholder="e.g. KA-01-AB-1234"
                ...
                <div class="md:col-span-2">
                    <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
                        Authorized Shopping Channels <span class="text-rose-400">*</span>
                    </label>
                    <p class="text-xs text-slate-500 mb-3">Check all channels this partner is cleared to deliver for (select at least one):</p>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        @foreach($allModes as $mode)
                            <label class="flex items-center gap-3 p-3.5 rounded-xl bg-slate-950 border border-slate-800 hover:border-slate-700 cursor-pointer transition">
                                <input type="checkbox" name="modes[]" value="{{ $mode->id }}"
                                    {{ in_array($mode->id, old('modes', $allModes->pluck('id')->toArray())) ? 'checked' : '' }}
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
            <div class="pb-4 border-b border-slate-800">
                ...
                <p class="text-xs text-slate-400 mt-1">Enter document numbers and optionally upload scans/photos for driving license and ID proof.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="license_number" id="license_number" value="{{ old('license_number') }}" placeholder="e.g. DL-1420110012345"
                        class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-white font-mono text-sm placeholder-slate-500 focus:outline-none transition">
                </div>
                ...
                <div>
                    ...
                    <span class="text-[10px] text-slate-500 mt-1 block">JPG, PNG, or WebP up to 4MB</span>
                </div>
                ...
                <div>
                    ...
                    <select name="id_proof_type" id="id_proof_type"
                        ...
                        <option value="voter_id" {{ old('id_proof_type') === 'voter_id' ? 'selected' : '' }}>Voter ID Card</option>
                ...
                <div>
                    ...
                    <input type="text" name="id_proof_number" id="id_proof_number" value="{{ old('id_proof_number') }}" placeholder="e.g. 1234-5678-9012"
                        class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-white font-mono text-sm placeholder-slate-500 focus:outline-none transition">
                </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="bank_account_number" id="bank_account_number" value="{{ old('bank_account_number') }}" placeholder="e.g. 100234567890"
                        class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-white font-mono text-sm placeholder-slate-500 focus:outline-none transition">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="bank_ifsc" id="bank_ifsc" value="{{ old('bank_ifsc') }}" placeholder="e.g. HDFC0001234"
                        class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-white font-mono text-sm placeholder-slate-500 focus:outline-none transition">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="upi_id" id="upi_id" value="{{ old('upi_id') }}" placeholder="e.g. partner@upi"
                        class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-white font-mono text-sm placeholder-slate-500 focus:outline-none transition">
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-4 pt-2">
            <a href="{{ route('admin.delivery_partners.index') }}" class="px-6 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-xs transition">
                Cancel
            </a>
