...
@section('title', 'Edit Mode: ' . $mode->name)
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl md:text-2xl font-black text-white tracking-tight">Edit Shopping Mode</h1>
                <p class="text-slate-400 text-xs mt-0.5">Modifying mode: <span class="text-indigo-400 font-semibold">{{ $mode->name }}</span> (ID #{{ $mode->id }})</p>
            </div>
        </div>
    </div>

    <!-- Mode Edit Form -->
    <form method="POST" action="{{ route('admin.modes.update', $mode) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
                    ...
                    <div>
                        ...
                        <input id="name" type="text" name="name" value="{{ old('name', $mode->name) }}" required
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label for="slug" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider">
                                URL Slug <span class="text-slate-500 font-normal font-mono">(Must be unique)</span>
                            </label>
                            <button type="button" id="unlockSlugBtn" class="text-[11px] text-indigo-400 hover:text-indigo-300 transition">
                                Regenerate from Name
                            </button>
                        </div>
                        <input id="slug" type="text" name="slug" value="{{ old('slug', $mode->slug) }}" required
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3"
                            ...
                            class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('description') border-rose-500 @enderror">{{ old('description', $mode->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative border-2 border-dashed border-slate-700 hover:border-indigo-500 rounded-2xl p-4 text-center cursor-pointer transition bg-slate-950" id="dropZone">
                            ...
                            <div id="uploadPlaceholder" class="space-y-2 {{ $mode->image_url ? 'hidden' : '' }}">
                            ...
                            <div id="imagePreviewContainer" class="{{ $mode->image_url ? '' : 'hidden' }}">
                                <img id="imagePreview" src="{{ $mode->image_url ?? '#' }}" alt="{{ $mode->name }}" class="w-full h-36 object-contain rounded-xl border border-slate-700 bg-slate-900/60 p-2">
                                <p class="text-[11px] text-indigo-400 font-medium mt-2">Click or drag to replace image</p>
                            </div>
                        </div>
                        @error('image')
                            <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input id="icon" type="text" name="icon" value="{{ old('icon', $mode->icon) }}"
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
                    ...
                    <div class="flex items-start gap-3 p-3.5 rounded-xl bg-slate-950 border border-slate-800">
                        <div class="flex items-center h-5">
                            <input id="status" type="checkbox" name="status" value="1" {{ old('status', $mode->status) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <input id="sort_order" type="number" name="sort_order" value="{{ old('sort_order', $mode->sort_order) }}" min="0"
