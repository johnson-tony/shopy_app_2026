                        ...
                        <tr class="hover:bg-slate-800/30 transition">
                            ...
                            <td class="py-3.5 px-4">
                                <form method="POST" action="{{ route('admin.categories.toggle-status', $category) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold transition cursor-pointer {{ $category->is_active ? 'bg-emerald-950/70 text-emerald-300 border border-emerald-500/40 hover:bg-emerald-900/60' : 'bg-rose-950/70 text-rose-300 border border-rose-500/40 hover:bg-rose-900/60' }}">
                                        <span class="w-1.5 h-1.5 rounded-full {{ $category->is_active ? 'bg-emerald-400' : 'bg-rose-400' }}"></span>
