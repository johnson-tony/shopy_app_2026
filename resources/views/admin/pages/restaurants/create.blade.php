...
@section('title', 'Register Restaurant Partner')
...
<div class="space-y-6 max-w-5xl">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl">
        <div>
            ...
            <h1 class="text-2xl font-black text-white tracking-tight">Register Restaurant Partner</h1>
            <p class="text-slate-400 text-sm mt-1">
                Add a new partner restaurant to the food delivery network with delivery settings and branding.
            </p>
        </div>
        <div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.restaurants.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
                    ...
                    <div class="space-y-1.5">
                        <label for="slug" class="block text-xs font-bold text-slate-300 uppercase tracking-wider">
                            URL Slug <span class="text-slate-500 normal-case font-normal">(Leave blank to auto-generate)</span>
                        </label>
                        <input type="text" 
                               name="slug" 
                               id="slug" 
                               value="{{ old('slug') }}" 
                               placeholder="thalappakatti-biryani" 
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="border-2 border-dashed border-slate-800 hover:border-amber-500/50 rounded-2xl p-4 text-center cursor-pointer transition bg-slate-950 relative overflow-hidden group">
                                ...
                                <div id="logoPreviewWrapper" class="hidden mb-2">
                                    <img id="logoPreviewImg" src="#" alt="Logo Preview" class="w-20 h-20 mx-auto rounded-xl object-cover border border-slate-700">
                                </div>
                                <div id="logoPlaceholder" class="space-y-1 py-3">
                                    <i class="fa-solid fa-cloud-arrow-up text-2xl text-slate-500 group-hover:text-amber-400 transition"></i>
                                    <p class="text-xs text-slate-400">Click to upload logo</p>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="border-2 border-dashed border-slate-800 hover:border-amber-500/50 rounded-2xl p-4 text-center cursor-pointer transition bg-slate-950 relative overflow-hidden group">
                                ...
                                <div id="bannerPreviewWrapper" class="hidden mb-2">
                                    <img id="bannerPreviewImg" src="#" alt="Banner Preview" class="w-full h-20 mx-auto rounded-xl object-cover border border-slate-700">
                                </div>
                                <div id="bannerPlaceholder" class="space-y-1 py-3">
                                    <i class="fa-solid fa-image text-2xl text-slate-500 group-hover:text-amber-400 transition"></i>
                                    <p class="text-xs text-slate-400">Click to upload banner</p>
            ...
            <div class="space-y-6">
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
                    ...
                    <label class="flex items-center justify-between p-3 rounded-2xl bg-slate-950 border border-slate-800 cursor-pointer hover:border-slate-700 transition">
                        ...
                        <input type="checkbox" name="is_pure_veg" value="1" {{ old('is_pure_veg') ? 'checked' : '' }} class="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500">
                    </label>
                    ...
                    <label class="flex items-center justify-between p-3 rounded-2xl bg-slate-950 border border-slate-800 cursor-pointer hover:border-slate-700 transition">
                        ...
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} class="w-4 h-4 rounded text-amber-600 focus:ring-amber-500">
                    </label>
                    ...
                    <label class="flex items-center justify-between p-3 rounded-2xl bg-slate-950 border border-slate-800 cursor-pointer hover:border-slate-700 transition">
                        ...
                        <input type="checkbox" name="status" value="1" {{ old('status', true) ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500">
                    </label>

                    <!-- Rating Defaults -->
                    <div class="grid grid-cols-2 gap-3 pt-2">
                        <div class="space-y-1">
                            <label class="text-[11px] font-bold text-slate-400 uppercase">Initial Rating</label>
                            <input type="number" step="0.1" min="1" max="5" name="rating" value="{{ old('rating', '4.5') }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-amber-500">
                        </div>
                        <div class="space-y-1">
                            <label class="text-[11px] font-bold text-slate-400 uppercase">Ratings Count</label>
                            <input type="number" min="0" name="ratings_count" value="{{ old('ratings_count', 120) }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-amber-500">
                        </div>
                    </div>
                </div>
